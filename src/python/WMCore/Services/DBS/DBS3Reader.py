...
class DBS3Reader:
    ...
    def listFileBlockLocation(self, fileBlockName, dbsOnly = False, phedex_nodes=False):
        ...
        for name, nodes in blockInfo.iteritems():
            ...
            for n in nodes:
                try:
                    cmsname(n) ## don't know how to distinguish between cmssite name and phedex node name here
                except AssertionError: ## is probably SE
                    if phedex_nodes: ## want phedexNode
                        n = (self.phedex.getNodeNames(n))
                else:  ## not SE i.e. phedexNode/cmssite
                    if not phedex_nodes:
                        n = self.phedex.getNodeSE(site) ## convert to SE
                final_nodes.add(n)
