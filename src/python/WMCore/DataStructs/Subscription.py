...
"""
__all__ = []
__revision__ = "$Id: Subscription.py,v 1.2 2008/07/04 16:45:47 metson Exp $"
...
class Subscription(Pickleable):
    ...
    def filesOfStatus(self, status=None):
        if status == 'GetAvailableFiles':
            return self.fileset - (self.acquired | self.completed | self.failed)
        elif status == 'GetAcquiredFiles':
            return self.acquired
        elif status == 'GetCompletedFiles':
            return self.completed
        elif status == 'GetFailedFiles':
            return self.failed
    ...
    def availableFiles(self):
        ...
        (e.g. not already in use)
        """
        return self.filestatus(status='GetAvailableFiles')
            
    def acquiredFiles(self):
        """
        Set of files marked as acquired.
        """
        return self.filestatus(status='GetAcquiredFiles')
        
    def completedFiles(self):
        """
        Set of files marked as completed.
        """
        return self.filestatus(status='GetCompletedFiles')
    
    def failedFiles(self):
        """
        Set of files marked as failed. 
        """
        return self.filestatus(status='GetFailedFiles')