...
class File_t(Base_t,TestCase):
    ...
    def testAdd(self):
        ...
        time = self.perfTest(dao=self.mysqldao, action='Files.Add', execinput=['files=self.testFile["lfn"]', 'size=self.testFile["size"]', 'events=self.testFile["events"]'])
    ...
    def testAddRunLumi(self): 
        ...
        time = self.perfTest(dao=self.mysqldao, action='Files.AddRunLumi', execinput=['files=self.testFileset.name', 'run=self.run', 'lumi=self.lumi'])
    ...
    def testAddToFileset(self):
        ...
        time = self.perfTest(dao=self.mysqldao, action='Files.AddToFileset', execinput=['file=self.testFile.lfn', 'fileset=self.testFileset.name '])
        assert time <= self.threshold, 'AddToFileset DAO class - Operation too slow ( elapsed time:'+str(time)+', threshold:'+str(self.threshold)+' )'

    def testDelete(self):
        print "testDelete"
        time = self.perfTest(dao=self.mysqldao, action='Files.Delete', execinput='file=self.testFile')
        assert time <= self.threshold, 'Delete DAO class - Operation too slow ( elapsed time:'+str(time)+', threshold:'+str(self.threshold)+' )'

    def testGetByID(self):
        print "testGetByID"
        time = self.perfTest(dao=self.mysqldao, action='Files.GetByID', execinput='files=self.testFile.id')
    ...
    def testGetByLFN(self):
        print "testGetByLFN" 
        time = self.perfTest(dao=self.mysqldao, action='Files.GetByLFN', execinput='files=self.testFile.lfn')
    ...
    def testGetLocation(self):
        ...
        time = self.perfTest(dao=self.mysqldao, action='Files.GetLocation', execinput='files=self.testFileset')
    ...
    def testGetParents(self):
        ...
        time = self.perfTest(dao=self.mysqldao, action='Files.GetParents', execinput='files=self.testFileset')
