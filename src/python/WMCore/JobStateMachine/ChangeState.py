...
from WMCore.Database.CMSCouch import CouchError, CouchNotFoundError, CouchServer
...
class ChangeState(WMObject, WMConnectionBase):
            ...
            if job.get("fwjr", None):
                ...
                if (job["retry_count"] > 0) or (newstate != 'success'):
                        ...
                        for singlefile in job["fwjr"].getAllFilesFromStep(step=singlestep):
                            if singlefile:
                                outputs.append({'type': 'output' if CMSSTEP.match(singlestep) else singlefile.get('module_label', None),
                                                ...
                                                'location': list(singlefile.get('locations', set([]))) if
                                                    len(singlefile.get('locations', set([]))) > 1 else
                                                    singlefile['locations'].pop(),
                    ...
                    jobSummary = {"_id": jobSummaryId,
                                  ...
                                  "acdc_url": "%s/%s" % (sanitizeURL(self.config.ACDC.couchurl)['url'],
                                                         self.config.ACDC.database),
    ...
    def loadExtraJobInformation(self, jobs):
        ...
        for idx, job in enumerate(jobs):
            ...
            if (job.get("task", None) is None or job.get("workflow", None) is None or
                job.get("taskType", None) is None or job.get("jobType", None) is None):
    ...
    def completeCreatedJobsInformation(self, jobs, incrementRetry=False):
        for job in jobs:
            ...
            if (job["mask"]["LastEvent"] is not None and
                job["mask"]["FirstEvent"] is not None and job["mask"]['inclusivemask']):
                job["nEventsToProc"] = int(job["mask"]["LastEvent"] - job["mask"]["FirstEvent"])
