...
__revision__ = "$Id: PhEDEx.py,v 1.1 2010/02/08 22:21:09 sryu Exp $"
...
class PhEDEx:
    ...
    def getReplicaInfoForBlocks(self, **args):
        ...
        files = self.dataBlocks.getFiles(args['block'])
        locations = self.dataBlocks.getLocation(args['block'])
        blocks.append({"files": len(files), "name": args['block'],
    ...
    def subscriptions(self, **args):
        ...
        if args.has_key('dataset') and args['dataset']:
            ...
            data['phedex']['dataset'].append({'name' : args['dataset'], 'files' : 5,
                                              'subscription' : []})
            ...
            dataset = self.dataBlocks.getDataset(args['block'])
            # TODO needs to add correct file numbers
