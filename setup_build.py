...
class BuildCommand(build_py):
    ...
    def compress_assets(self):
        if not self.compress:
            for dir, files in self.distribution.package_data:
                for f in files:
                    if f.find("-min.") >= 0:
                        print("removing", f)
                        os.remove(f)
...
class InstallCommand(install):
    ...
    def finalize_options(self):
        ...
        self.distribution.packages, self.distribution.py_modules = things_to_build(self)
        self.distribution.package_data = list_static_files(dependencies[self.system])
        docroot = "%s/doc/build/html" % get_path_to_wmcore_root()
        for dirpath, _, files in os.walk(docroot):
            self.distribution.package_data.append(("doc%s" % dirpath[len(docroot):],
                                                   ["%s/%s" % (dirpath, fname) for fname in files if
                                                    fname != '.buildinfo']))
        # Mangle data paths if patching.
        if self.patch:
            self.distribution.package_data = [('x' + dir, files) for dir, files in self.distribution.package_data]
