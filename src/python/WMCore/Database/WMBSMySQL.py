...
__revision__ = "$Id: WMBSMySQL.py,v 1.1 2008/04/10 19:45:10 evansde Exp $"
__version__ = "$Revision: 1.1 $"

from WMCore.Database.DBCore import wmSQL


class wmbsMySQL(wmSQL):   
    """
    ...
    def __init__(self, logger, engine):
        ...
        self.create['wmbs_subscription'] = """CREATE TABLE wmbs_subscription (
                ...
                FOREIGN KEY(fileset) REFERENCES wmbs_fileset(id)
                    ON DELETE CASCADE)"""
        self.create['wmbs_sub_files_acquired'] = """CREATE TABLE wmbs_sub_files_acquired (
                subscription INT(11) NOT NULL,
                file         INT(11) NOT NULL,
                FOREIGN KEY (subscription) REFERENCES wmbs_subscription(id)
                    ON DELETE CASCADE,
                FOREIGN KEY (file) REFERENCES wmbs_file(id))"""
        self.create['wmbs_sub_files_failed'] = """CREATE TABLE wmbs_sub_files_failed (
                subscription INT(11) NOT NULL,
                file         INT(11) NOT NULL,
                FOREIGN KEY (subscription) REFERENCES wmbs_subscription(id)
                    ON DELETE CASCADE,
                FOREIGN KEY (file) REFERENCES wmbs_file(id))"""
        self.create['wmbs_sub_files_complete'] = """CREATE TABLE wmbs_sub_files_complete (
                subscription INT(11) NOT NULL,
                file         INT(11) NOT NULL,
                FOREIGN KEY (subscription) REFERENCES wmbs_subscription(id)
                    ON DELETE CASCADE,
        ...
        self.insert['fileset'] = "insert into wmbs_fileset (name) values (:fileset)"
        self.insert['newfile'] = """insert into wmbs_file_details (lfn, size, events, run, lumi) 
                values (:lfn, :size, :events, :run, :lumi)"""
        self.insert['fileforfileset'] = """insert into wmbs_fileset_files (file, fileset) 
            values ((select id from wmbs_file_details where lfn = :file),
            (select id from wmbs_fileset where name = :fileset))"""
        self.insert['newsubscription'] = """insert into wmbs_subscription (fileset, type) 
                values ((select id from wmbs_fileset where name =:fileset), :type)"""
        self.insert['newlocation'] = "insert into wmbs_location (se_name) values (:location)"
        self.insert['putfileatlocation'] = """insert into wmbs_file_location (file, location) 
                values ((select id from wmbs_file_details where lfn = :file),
                (select id from wmbs_location where se_name = :location))"""
        #TODO: The following should be bulk inserts
        self.insert['acquirefiles'] = "insert into wmbs_sub_files_acquired (subscription, file) values (:subscription, :file)"
        self.insert['failfiles'] = "insert into wmbs_sub_files_failed (subscription, file) values (:subscription, :file)"
        self.insert['completefiles'] = "insert into wmbs_sub_files_complete (subscription, file) values (:subscription, :file)"
                        
        self.select['allfileset'] = "select * from wmbs_fileset order by last_update, name"
        self.select['filesinfileset'] = """select id, lfn, size, events, run, lumi from wmbs_file_details 
                where id in (select file from wmbs_fileset_files where 
                fileset = (select id from wmbs_fileset where name = :fileset))"""
        self.select['subscriptionsoftype'] = "select id, fileset from wmbs_subscription where type=:type"
        self.select['subscriptionsforfileset'] = """select id, type from wmbs_subscription 
                where fileset=(select id from wmbs_fileset where name =:fileset)"""
        self.select['subscriptionsforfilesetoftype'] = """select id from wmbs_subscription where type=:type 
                and fileset=(select id from wmbs_fileset where name =:fileset)"""
        self.select['filesetforsubscription'] = """select name from wmbs_fileset 
                where id = (select fileset from wmbs_subscription where id = :subscription)""" 
        self.select['alllocations'] = "select id, se_name from wmbs_location"
        self.select['filesatlocation'] = """select id, lfn, size, events, run, lumi from wmbs_file_details 
        ...
        self.select['activefiles'] = """select file from wmbs_sub_files_acquired 
...
# A file that has failed as part of a subscription processing task.
        self.processData(self.create['wmbs_sub_files_failed'])
        
    def createSubscriptionCompletedFilesTable(self): 
# A table of files that have been successfully processed by the subscription
...
# Link a file to a job instance
    ...
    def insertFileset(self, fileset = None, conn = None, transaction = False):
        ...
        self.processData(self.insert['fileset'], binds, conn = conn, transaction = transaction)
    ...
    def showFilesInFileset(self, fileset = None, conn = None, transaction = False):
        binds = {'fileset' : fileset}
        return self.processData(self.select['filesinfileset'], binds, conn = conn, transaction = transaction)
            
    def newSubscription(self, fileset = None, subtype='processing', conn = None, transaction = False):
        if type(fileset) == type('string'):
            binds = {'fileset': fileset, 'type': subtype}
            self.processData(self.insert['newsubscription'], binds, conn = conn, transaction = transaction)
    ...
    def subscriptionsForFileset(self, fileset = None, subtype=None, conn = None, transaction = False):
        if fileset:
            if subtype:
                binds = {'fileset' : fileset, 'type':subtype}
                return self.processData(self.select['subscriptionsforfilesetoftype'], binds, conn = conn, transaction = transaction)
            else:
                binds = {'fileset' : fileset}
                return self.processData(self.select['subscriptionsforfileset'], binds, conn = conn, transaction = transaction)
            
    def listSubscriptionsOfType(self, subtype=None, conn = None, transaction = False):
        if subtype:
            binds = {'type':subtype}
            return self.processData(self.select['subscriptionsoftype'], binds, conn = conn, transaction = transaction)

    def filesForSubscription(self, subscription=0, conn = None, transaction = False):
        if subscription > 0:
            binds ={'subscription':subscription}
            fileset=self.processData(self.select['filesetforsubscription'], binds, conn = conn, transaction = transaction)
            fs = ''
            for f in fileset:
                fs = str(f.fetchone()[0])
    ...
    def addNewLocation(self, sename = None, conn = None, transaction = False):
        ...
        if not isinstance(sename, list): sename = [sename]
        binds = []
        for s in sename:
            binds.append({'location':s})
        return self.processData(self.insert['newlocation'], binds, conn = conn, transaction = transaction)
    
    def listAllLocations(self, conn = None, transaction = False):
        return self.processData(self.select['alllocations'], conn = conn, transaction = transaction)
    ...
    def listFilesAtLocation(self, sename, conn = None, transaction = False):
        ...
        for s in sename:
            binds.append({'location':s})
        return self.processData(self.select['filesatlocation'], binds, conn = conn, transaction = transaction)
                                
    def putFileAtLocation(self, file = None, sename=None, conn = None, transaction = False):
        ...
        return self.processData(self.insert['putfileatlocation'], binds, conn = conn, transaction = transaction)
    ...
    def locateFile(self, file, conn = None, transaction = False):
        ...
        for f in file:
            binds.append({'file':f})
        return self.processData(self.select['whereisfile'], binds, conn = conn, transaction = transaction)
    
    def listAvailableFiles(self, subscription=None, conn = None, transaction = False):
        if isinstance(subscription, int):
            return self.processData(self.select['nextfiles'], {'subscription':subscription})
        else:
    ...
    def acquireNewFiles(self, subscription=0, files=None, conn = None, transaction = False):
        if isinstance(subscription, int):
            binds = []
            if not isinstance(files, list): files = [files]
            for f in files:
                binds.append({'subscription':subscription, 'file':f})                
            return self.processData(self.insert['acquirefiles'], binds)
            
        else:
            self.logger.exception("acquireNewFiles requires a single id number for subscription")
            raise "non-integer subscription id given"

    def failFiles(self, subscription=0, files=None, conn = None, transaction = False):
        if isinstance(subscription, int):
            binds = []
            if not isinstance(files, list): files = [files]
            for f in files:
    ...
    def completeFiles(self, subscription=0, files=None, conn = None, transaction = False):
        if isinstance(subscription, int):
            binds = []
            if not isinstance(files, list): files = [files]
            for f in files:
    ...
    def listAcquiredFiles(self, subscription=None, conn = None, transaction = False):
        if isinstance(subscription, int):
            return self.processData(self.select['activefiles'], {'subscription':subscription})
        else:
            self.logger.exception("listActiveFiles requires a single id number for subscription")
            raise "non-integer subscription id given"    
    
    def listFailedFiles(self, subscription=None, conn = None, transaction = False):
        if isinstance(subscription, int):
            return self.processData(self.select['failedfiles'], {'subscription':subscription})
        else:
            self.logger.exception("listFailedFiles requires a single id number for subscription")
            raise "non-integer subscription id given"
    
    def listCompletedFiles(self, subscription=None, conn = None, transaction = False):
        if isinstance(subscription, int):
            return self.processData(self.select['completedfiles'], {'subscription':subscription})
        else:
            self.logger.exception("listFailedFiles requires a single id number for subscription")
            raise "non-integer subscription id given"
...
###
    ...
    def addNewFilesetToLocation(self, file=None, fileset=None, sename=None, conn = None, transaction = False):
        """
        ...
        try:
            self.insertFileset(fileset=fileset, conn = conn, transaction=trans)
            self.insertFilesForFileset(files=file, fileset=fileset, conn = conn, transaction=trans)
            self.putFileAtLocation(file, sename, conn = conn, transaction=trans)
            if not transaction: trans.commit()
        except Exception, e:
            if not transaction: trans.rollback()
            raise e
        
    def addNewFileToLocation(self, file=None, fileset=None, sename=None, conn = None, transaction = False):
        """
