...
def fwjr_parser(doc):
    ...
    for key, val in list(steps.items()):
        ...
        if  key.startswith('cmsRun'):
            ...
            odict = val['output']
            for kkk, vvv in list(odict.items()):
                for row in vvv:
                    if  row.get('merged', False):
                        ...
                        totalLumis = sum([len(r) for r in list(row['runs'].values())])
                        dataset_summary = \
...
def merge_docs(doc1, doc2):
    ...
    for key in ['_id', '_rev']:
        ...
        if  key in doc2:
            del doc2[key]
    for key, val in list(doc1.items()):
        if  key not in doc2:
            return False
...
def update_tasks(old_tasks, new_tasks):
    ...
    for task, tval in list(new_tasks.items()):
        if  task in old_tasks:
            for site, sval in list(tval['sites'].items()):
