...
if [[ $rc != 0 ]]
then
	# python2.6 isn't here, go through (hopefully to be removed) site-specific setup
	echo "WMAgent couldn't find python2.6, attempting site-specific magic"
	if [ -e /uscmst1/prod/sw/cms/slc5_amd64_gcc434/external/python/2.6.4-cms2/etc/profile.d/init.sh ]
	then
		# FNAL-specific setup
		echo "WMAgent thinks it's running at FNAL, trying FNAL-specific bootstrapping"
...
echo "WMAgent's gonna take a nap to waste some time"
echo "WMAgent wonders if norman, foulkes or evans are responsible for this behavior"
sleep 90
