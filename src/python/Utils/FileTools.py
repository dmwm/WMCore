...
def loadEnvFile(wmaEnvFilePath, logger=None):
    ...
    completedProc = subprocess.run(['bash', '-c', f'source {wmaEnvFilePath} && python -c "import os; print(repr(os.environ.copy()))" '],
                                   capture_output=True)
    if completedProc.returncode == 0:
        newEnv = eval(completedProc.stdout)
        os.environ.update(newEnv)
        if completedProc.stderr:
            logger.warning("Environment file: %s loaded with errors:", wmaEnvFilePath)
            logger.warning(completedProc.stderr.decode())
        else:
        ...
        logger.error(completedProc.stderr.decode())
        return False
