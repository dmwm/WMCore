...
from OldMsgService import OldMsgService
...
class ProxyTest(unittest.TestCase):
    ...
    def testA(self):
        ...
        details['subscription'] = ['Logging.DEBUG','Logging.INFO','Logging.ERROR','Logging.NOTSET','LogState','Stop','StopAndWait','JobSuccess','JobFailure','JobCreate']
        ...
        print('--Create a new msgService instance (these represent other components in our test)\n')
        myThread = threading.currentThread()
        ...
        print('--New msgService subscribes to JobSuccess and JobFailure (these are proxy subscribed to by our proxy comonent)\n')
        myThread.transaction.begin()
        newMsgService.subscribeTo("JobSuccess")
        newMsgService.subscribeTo("JobFailure")
        myThread.transaction.commit()
        print('--Waiting a few seconds to make sure the (proxy) subscriptions arrived\n')
        time.sleep(10)
        # now we can publish a message in the new component that will be forwarded by the proxy
        myThread.transaction.begin()
        ...
        # as we are running the component in a thread we want the component to stop
        # once all its threads minus itself and the main thread are done
